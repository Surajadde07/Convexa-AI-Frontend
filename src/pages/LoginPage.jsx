...
// ─────────────────────────────────────────
//  GOOGLE BUTTON — reusable across Login & Signup
// ─────────────────────────────────────────
// Uses the Google Identity Services (GIS) script injected once per page load.
// The script is loaded lazily on first render via useEffect so we don't block
// the page if the CDN is slow.
...
function GoogleAuthButton({ label, onSuccess, onError }) {
  ...
  const handleClick = useCallback(() => {
    ...
    // Show the native Google One Tap or fall back to the popup
    window.google.accounts.id.prompt((notification) => {
      if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
        // One Tap suppressed — open the full sign-in popup instead
  ...
  return (
    <>
      {/* Hidden div used by GIS renderButton fallback */}
